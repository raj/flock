module Flock
  ...
  struct Mat4
    ...
    # Full 4x4 inverse (column-major adjugate / determinant). Returns identity if singular.
    def inverse : Mat4
      m = @m
      r = StaticArray(Float32, 16).new(0.0f32)
      ...
      return Mat4.identity if det.abs < 1e-12f32
      d = 1.0f32 / det
      16.times { |i| r[i] = r[i] * d }
      Mat4.new(r)
    end
