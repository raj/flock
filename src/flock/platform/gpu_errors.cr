module Flock
  ...
  # Creates the instance/adapter/device/queue and returns a surface-less `GpuContext`.
  # The raw handles stay reachable through it (`gpu.instance`, `gpu.device`, `gpu.queue`):
  #   gpu = Flock.headless_context(SIZE, SIZE)
  #   renderer = Flock::Renderer3D.new(gpu)
  def self.headless_context(width : Int, height : Int,
                            format : LibWGPU::TextureFormat = LibWGPU::TextureFormat::RGBA8Unorm) : GpuContext
    ...
    GpuContext.new(instance, adapter, device, queue,
