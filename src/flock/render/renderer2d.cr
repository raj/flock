module Flock
  ...
  class Renderer2D < Resource
    ...
    # `window` selects which cameras render here: only cameras whose `window` matches (default
    # 0 = the primary window). Secondary windows pass their `slot` (see MultiWindowPlugin).
    def render_into(target : LibWGPU::TextureView, width : UInt32, height : UInt32, world : World,
                    load_previous : Bool = false, window : Int32 = 0) : Nil
      @last_draw_calls = 0
      cameras = [] of Camera2D
      world.query(Camera2D) { |_e, cam| cameras << cam.value if cam.value.active && cam.value.window == window }
