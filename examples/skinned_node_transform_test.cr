...
def g(px : UInt8*, x : Int32, y : Int32, rb : UInt32)
  px[y * rb.to_i + x * 4 + 1].to_i
end
left = g(px, 8, 32, rb)   # where x=-2 projects (quad expected here with the fix)
center = g(px, 32, 32, rb) # x=0 (background with the fix; quad with the old code)
